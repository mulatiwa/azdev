...
import Approach, { APPROACH_FRAGMENT } from './Approach.js';
import NewApproach from './NewApproach.js';
...
export default function TaskPage({ taskId }) {
  ...
  useEffect(() => {
    if(!taskInfo)
    {
      request(TASK_INFO, { variables: { taskId } })
        .then(({ data }) => setTaskInfo(data.taskInfo))
    }
  }, [request, taskId, taskInfo])
