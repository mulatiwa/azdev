...
import { APPROACH_FRAGMENT } from './Approach.js';
...
export default function NewApproach({ taskId, onSuccess }) {
  ...
  useEffect(() => {
    ...
    {
      request(DETAIL_CATEGORIES)
        .then(({ data }) => setDetailCategories(data.detailCategories.enumValues))      
    }
  ...
  const handleNewApproachSubmit = async e => 
  {
    e.preventDefault();
    setUIErrors([]);
    ...
    const { data, errors: rootErrors } = await request(APPROACH_CREATE,
      {
        variables:{
          taskId,
          input:
