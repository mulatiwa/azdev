...
// import { gqlEndPoint } from './config/api-config.js';
...
export const useStoreObject = () => {
  ...
  const request = async (requestText, { variables } = {}) => {
    const headers = (state.user) ? { Authorization: `Bearer ${state.user.authToken}` } : { };
    const gsResp = await fetch('http://localhost:9550/graphql',
      {
        method: 'post',
